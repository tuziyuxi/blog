...
Kafka数据从生产者推送到代理并由消费者从代理提取

* 消费者立场

主题分为一组完全有序的分区，每个分区在任何给定时间由每个订阅消费者组中的一个消费者消费。这意味着每个分区中的使用者的位置
只是一个整数，即消耗的下一个消息的偏移量。

消费者可以故意回退到旧的偏移并重新使用数据。
